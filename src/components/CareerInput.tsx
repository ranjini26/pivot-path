...
export const CareerInput = ({
  ...
  const examplePrompts = ["I'm a retail manager but stores are closing. I feel lost about what's next.", "Been in marketing for 10 years, got laid off. Not sure if I should pivot or stay.", "I'm a teacher but burnt out. Want to use my skills differently but don't know how."];
      ...
      <Card className="bg-white dark:bg-gray-900 p-8 border border-gray-200 dark:border-gray-700 shadow-lg">
        <div className="mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <Textarea value={input} onChange={e => setInput(e.target.value)} placeholder="I'm currently working as... but I'm feeling stuck because..." className="min-h-[140px] resize-none border-2 border-gray-300 dark:border-gray-600 focus:border-indigo-500 dark:focus:border-indigo-400 transition-colors text-base text-gray-900 dark:text-white bg-white dark:bg-gray-800 placeholder:text-gray-500 dark:placeholder:text-gray-400" disabled={isLoading} />
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-4">
                <label className="cursor-pointer">
                  <input type="file" accept=".pdf,image/*" onChange={handleResumeUpload} className="hidden" disabled={isLoading} />
                ...
                {resume && <div className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
            ...
            <Button type="submit" disabled={!input.trim() || isLoading} className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white hover:from-indigo-600 hover:to-purple-700 transition-all duration-200 font-medium py-3 text-base disabled:opacity-50">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {examplePrompts.map((prompt, index) => <button key={index} onClick={() => setInput(prompt)} disabled={isLoading} className="w-full text-left p-4 rounded-lg bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-sm border border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500 disabled:opacity-50 disabled:cursor-not-allowed text-gray-800 dark:text-gray-200">
                "{prompt}"
              </button>)}
          </div>
        </div>
